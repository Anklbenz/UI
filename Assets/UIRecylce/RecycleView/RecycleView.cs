...
public class RecycleView : UnlockedScrollRect {
	...
	private float realContentHeight => recycleDataSource.count * _itemHeight;
	...
	public IEnumerator Reload() {
		...
		var newPoolSize = GetRequiredPoolSize();
		var scrollContentSizeY = newPoolSize * _itemHeight;
		content.sizeDelta = new Vector2(content.sizeDelta.x, scrollContentSizeY);

		if (newPoolSize > _visibleItemsPoolSize)
			IncreasePool(newPoolSize);
		else
			DecreasePool(newPoolSize);

		_visibleItemsPoolSize = newPoolSize;
	...
	private int GetRequiredPoolSize() {
		...
		//float requiredContentHeight = VIEWPORT_TO_POOL_RATIO * viewport.rect.height;
